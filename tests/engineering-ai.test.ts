...
test("canonical policy encodes calculation, missing-data, units, assumptions, material, coefficient, formula, design, and sanity safeguards", () => {
  ...
  for (const required of [
    ...
    /clearly labeled approximate and educational/iu,
...
test("hallucination trap set resolves to an engineering control rather than an unsupported answer path", () => {
  const traps: Array<{ prompt: string; expected: EngineeringIntent; control: RegExp }> = [
    ...
    { prompt: "Give the exact yield strength of unknown alloy X", expected: "ENGINEERING_CONCEPT", control: /authoritative source/iu },
...
test("off-topic route bypasses provider and lookup but persists one normal idempotent exchange", async () => {
  const state = { providerCalls: 0, lookupCalls: 0, completions: 0, xp: 0, persisted: "" };
  const repository = {
    ...
    async completeExchange(_userId: string, _token: string, _session: string, requestId: string, text: string, _module: string, xp: number) {
      state.completions += 1; state.xp = xp; state.persisted = text;
      return { userMessage: { id: "u", sender: "user" as const, text: "Write me a love poem", module: "tutor" as const, timestamp: new Date().toISOString() }, assistantMessage: { id: "a", sender: "ai" as const, text, module: "tutor" as const, timestamp: new Date().toISOString(), requestId }, progress: { xp, level: 1, streak: 1, requests_count: 1, material_count: 0, patent_count: 0, modules_used: ["tutor"] }, awarded: true };
    },
  ...
  assert.deepEqual(state, { providerCalls: 0, lookupCalls: 0, completions: 1, xp: 15, persisted: engineeringOffTopicRedirect("en") });
});
