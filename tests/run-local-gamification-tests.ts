...
if (status.status !== 0) {
  process.stderr.write("Local Supabase is unavailable. Start the local stack before running gamification tests.\n");
...
if (!apiUrl || !publishableKey || !secretKey || !/^https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?$/i.test(apiUrl)) {
  process.stderr.write("Local Supabase did not return a safe loopback configuration.\n");
  process.exit(1);
}

const result = spawnSync(process.execPath, [path.resolve("node_modules", "tsx", "dist", "cli.mjs"), "--test", path.resolve("tests", "integration", "gamification.test.ts")], {
  stdio: "inherit",
