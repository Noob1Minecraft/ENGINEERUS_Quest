...
if (!apiUrl || !publishableKey || !secretKey || !/^https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?$/iu.test(apiUrl)) {
  process.stderr.write("Safe local document test configuration is unavailable.\n");
  process.exit(1);
}
const tsxCli = path.resolve("node_modules", "tsx", "dist", "cli.mjs");
const result = spawnSync(process.execPath, [tsxCli, "--test", path.resolve("tests", "integration", "documents.test.ts")], {
  stdio: "inherit",
