-- Make the general authenticated API budget authoritative across backend
-- instances while preserving the existing data-minimal UUID-only counter.
...
begin
  ...
  v_limit := case p_operation
    when 'authenticated_general' then 180
...
comment on function public.consume_abuse_budget(uuid, text) is
  'Authoritative fixed-window budgets for general authenticated API traffic and sensitive operations; stores no request content.';
