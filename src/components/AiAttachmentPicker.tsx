...
const copy = {
  ru: { attach: 'Добавить контекст', document: 'Документ', image: 'Изображение', library: 'Выберите готовый файл', upload: 'Загрузить новый', empty: 'Готовых файлов пока нет.', loading: 'Загрузка…', failed: 'Не удалось загрузить список файлов.', processing: 'Файл загружен и обрабатывается. Выберите его после завершения обработки.', maxImages: 'К сообщению можно прикрепить не более трёх изображений.', close: 'Закрыть' },
  kk: { attach: 'Контекст қосу', document: 'Құжат', image: 'Сурет', library: 'Дайын файлды таңдаңыз', upload: 'Жаңасын жүктеу', empty: 'Дайын файлдар әлі жоқ.', loading: 'Жүктелуде…', failed: 'Файлдар тізімін жүктеу мүмкін болмады.', processing: 'Файл жүктелді және өңделуде. Өңдеу аяқталғаннан кейін таңдаңыз.', maxImages: 'Хабарламаға үш суреттен артық тіркеуге болмайды.', close: 'Жабу' },
  en: { attach: 'Add context', document: 'Document', image: 'Image', library: 'Choose a ready file', upload: 'Upload new', empty: 'No ready files yet.', loading: 'Loading…', failed: 'Files could not be loaded.', processing: 'The file was uploaded and is processing. Select it after processing completes.', maxImages: 'Up to three images can be attached to one message.', close: 'Close' },
        ...
        <div role="dialog" aria-label={labels.attach} className="absolute bottom-14 left-0 z-30 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-3 shadow-xl">
          ...
          {!mode ? (
            ...
            <div className="mt-3 space-y-2">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-slate-600">{labels.library}</span><button type="button" onClick={() => (mode === 'document' ? documentInput : imageInput).current?.click()} className="flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200"><Upload className="h-3.5 w-3.5" />{labels.upload}</button></div>
