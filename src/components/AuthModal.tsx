      ...
      <div ref={dialogRef} tabIndex={-1} className="eq-dialog eq-auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-dialog-title">
        ...
        {view === 'forgot' || view === 'request-sent' ? <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="eq-form-field"><span className="eq-auth-field-heading"><label htmlFor="auth-password">{copy.password}</label>{view === 'login' && <button type="button" className="eq-auth-link" onClick={() => selectView('forgot')}>{copy.forgotPassword}</button>}</span><span className="eq-auth-input-wrap"><Lock aria-hidden="true" /><input id="auth-password" type="password" required value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={view === 'register' ? 'new-password' : 'current-password'} /></span></div>
