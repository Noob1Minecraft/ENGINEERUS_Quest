...
export const DocumentsTab: React.FC<Props> = ({ authenticated, lang, onRequireAuth, onUseWithTutor, onUseImagesWithTutor }) => {
  ...
  if (!authenticated) return <section className="rounded-2xl border border-slate-200 bg-white p-8 text-center"><LockKeyhole aria-hidden="true" className="mx-auto h-9 w-9 text-blue-600" /><p className="mt-3 font-bold text-slate-800">{labels.login}</p><button type="button" onClick={onRequireAuth} className="mt-4 rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">{labels.signIn}</button></section>;

  return <div className="space-y-7">
    <header className="border-b border-slate-200 pb-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{labels.title}</p><h1 className="mt-2 text-2xl font-black text-slate-950 sm:text-3xl">{labels.subtitle}</h1><p className="mt-3 flex max-w-3xl items-start gap-2 text-sm leading-relaxed text-slate-600"><LockKeyhole aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-blue-600" />{labels.private}</p></header>
