...
const COPY = {
  ru: {
    ...
    ready: 'Готов', processing: 'Обработка', failed: 'Ошибка', ocr: 'Нужен текстовый PDF', uploaded: 'Загружен', uploadSuccess: 'Документ добавлен в частную библиотеку.', pages: 'стр.', safety: 'Перед инженерным решением сверяйте ответ AI Tutor с исходным документом.', name: 'Имя', type: 'Тип', size: 'Размер', updated: 'Обновлён', status: 'Статус', actions: 'Действия',
  },
  kk: {
    ...
    deleteTitle: 'Құжат жойылсын ба?', deleteDescription: 'Файл жеке кітапханадан өңделген контекстімен бірге жойылады. AI Tutor-дың бұрынғы жауаптары тарихта қалады.', cancel: 'Бас тарту', deleting: 'Жойылуда…', deleteConfirm: 'Құжатты жою', deleteError: 'Құжат жойылмады. Файл кітапханада қалды — қайталап көріңіз.', deleted: 'Құжат жеке кітапханадан жойылды.', ready: 'Дайын', processing: 'Өңделуде', failed: 'Қате', ocr: 'Мәтіндік PDF қажет', uploaded: 'Жүктелді', uploadSuccess: 'Құжат жеке кітапханаға қосылды.', pages: 'бет', safety: 'Инженерлік шешім алдында AI Tutor жауабын бастапқы құжатпен салыстырыңыз.', name: 'Атауы', type: 'Түрі', size: 'Өлшемі', updated: 'Жаңартылды', status: 'Күйі', actions: 'Әрекеттер',
  },
  en: {
...
export const DocumentsTab: React.FC<Props> = ({ authenticated, lang, onRequireAuth, onUseWithTutor, onUseImagesWithTutor }) => {
  ...
  return <div className="eq-library-workspace space-y-7">
    ...
    <section aria-labelledby="documents-heading" className="eq-library-section space-y-4">
      ...
      {loading ? <div role="status" className="flex items-center gap-2 py-8 text-sm font-semibold text-slate-500"><Loader2 aria-hidden="true" className="h-5 w-5 animate-spin text-blue-600" />{labels.loading}</div> : documents.length === 0 ? <div className="border-y border-dashed border-slate-300 py-10 text-center"><FileText aria-hidden="true" className="mx-auto h-8 w-8 text-blue-500" /><h3 className="mt-3 text-base font-black text-slate-800">{labels.emptyTitle}</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-slate-500">{labels.empty}</p></div> : <div className="eq-document-table" role="table" aria-label={labels.documents}>
        <div className="eq-document-table__head" role="row"><span role="columnheader">{labels.name}</span><span role="columnheader">{labels.type}</span><span role="columnheader">{labels.size}</span><span role="columnheader">{labels.updated}</span><span role="columnheader">{labels.status}</span><span role="columnheader">{labels.actions}</span></div>
        {/* Filename cells retain overflow-wrap:anywhere through the table stylesheet. */}
        <div role="rowgroup">{documents.map((document) => { const status = documentStatus(document, lang); return <article key={document.id} className="eq-document-row" role="row"><div className="eq-document-row__name" role="cell"><FileText aria-hidden="true" /><span><strong>{document.original_filename}</strong>{document.page_count ? <small>{document.page_count} {labels.pages}</small> : null}{document.status === 'failed' ? <small className="text-rose-700">{labels.parseError}</small> : null}</span></div><span role="cell" data-label={labels.type}>{document.file_type.toUpperCase()}</span><span role="cell" data-label={labels.size}>{formatBytes(document.size_bytes, lang)}</span><time role="cell" data-label={labels.updated} dateTime={document.created_at}>{formatDate(document.created_at, lang)}</time><span role="cell" data-label={labels.status} className={`eq-document-row__status ${status.tone}`}>{status.label}</span><div role="cell" className="eq-document-row__actions"><button type="button" disabled={document.status !== 'ready'} onClick={() => onUseWithTutor(document)}><Paperclip aria-hidden="true" />{labels.attach}</button><button type="button" onClick={() => setPendingDelete(document)}><Trash2 aria-hidden="true" />{labels.remove}</button></div></article>; })}</div>
      </div>}
    </section>
