...
export const ImagesPanel: React.FC<Props> = ({ lang, onUseWithTutor }) => {
  ...
  return <section aria-labelledby="images-heading" className="eq-image-library space-y-4 border-t border-slate-200 pt-7">
    {/* Image captions retain overflow-wrap:anywhere through the compact-grid stylesheet. */}
    <div className="eq-image-toolbar"><div><h2 id="images-heading">{labels.title}</h2><p>{labels.subtitle}</p><small>{labels.note}</small></div><div className="eq-image-toolbar__actions"><span>{images.length} / 30</span><input ref={inputRef} aria-label={labels.upload} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void onFile(event.target.files?.[0])} /><button type="button" disabled={uploading} onClick={() => inputRef.current?.click()}>{uploading ? <Loader2 aria-hidden="true" className="animate-spin" /> : <Upload aria-hidden="true" />}{uploading ? labels.uploading : labels.upload}</button></div></div>
    ...
    {loading ? <div role="status" className="flex items-center gap-2 py-8 text-sm font-semibold text-slate-500"><Loader2 aria-hidden="true" className="h-5 w-5 animate-spin text-blue-600" />{labels.loading}</div> : images.length === 0 ? <div className="border-y border-dashed border-slate-300 py-10 text-center"><ImageIcon aria-hidden="true" className="mx-auto h-8 w-8 text-indigo-500" /><h3 className="mt-3 text-base font-black text-slate-800">{labels.emptyTitle}</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-slate-500">{labels.empty}</p></div> : <div className="eq-image-grid">
      {images.map((image) => { const isSelected = selected.includes(image.id); return <figure key={image.id} className={`eq-image-tile${isSelected ? ' is-selected' : ''}`}><button type="button" onClick={() => toggle(image)} disabled={image.status !== 'ready'} aria-pressed={isSelected} className="eq-image-tile__select"><span className="eq-image-tile__canvas">{isSelected ? <Check aria-hidden="true" /> : <ImageIcon aria-hidden="true" />}</span><span className="eq-image-tile__status">{statusLabel(image, lang)}</span><span className="sr-only">{image.original_filename}</span></button><figcaption><strong>{image.original_filename}</strong><span>{image.width}×{image.height} · {formatBytes(image.size_bytes, lang)}</span><time dateTime={image.created_at}>{formatDate(image.created_at, lang)}</time></figcaption><button type="button" onClick={() => setPendingDelete(image)} className="eq-image-tile__delete"><Trash2 aria-hidden="true" />{labels.remove}</button></figure>; })}
    </div>}
